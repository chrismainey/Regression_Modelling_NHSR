...
<br>
Usually in "strength" and "direction" of association
...
+ ___Regression:___ estimate how one variable: $y$ changes when we observed changes in another: $x$ (or more than one)
...
+ _Take away the mean, and divide by standard deviation (z-score)._
...
+ $\beta$ becomes the change in $y$ for each standard deviation increase in $x$ 
...
```{r lmplot, echo=FALSE, fig.retina=2, fig.align="center", fig.height=5.5, fig.width=8, fig.keep = 'last'}
par(mfrow=c(2,2))
plot(z)
```
...
+ Models won't understand text
+ If we provide a numeric category, it will see it as numeric
--

+ `R` has a built in data type: `factors`

`Factors` are 'dummy coded:' _pivotted to binary columns_
--

+ If we have a categories: "A", "B" & "C", we get:
```{r dummy, echo}
...
# Exercise 5: Predicting 10-year CHD in Framingham data
