...
## Example:
```{r lm1, fig.retina= 2, fig.align="center", fig.height=5, fig.width=6}
x <- rnorm(50, 10, 4)
y <- runif(50, min = 0.5, 10) + (1.25*x)
...
```{r lm3, fig.retina=2, fig.align="center", fig.height=4, fig.width=5}
z <- lm(y~x)
...
- Uses the residual error (ε) to work out the position of the line.
--

- 'The 'Residual' is the distance between the prediction and the data point.
--

- Squared it (or sum would be zero) and minimise 'sum of the squares'
--

```{r lm2, fig.retina=2, fig.align="center", fig.height=4, fig.width=5}
...
```{r lmplot, fig.retina=2, fig.align="center", fig.height=5, fig.width=8, fig.keep = 'last'}
par(mfrow=c(2,2))
plot(z)
```
