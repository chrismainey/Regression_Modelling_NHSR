...
hist(framingham$sysBP)

BPmod1 <- lm(sysBP ~ ., data = framingham) 

summary(BPmod1)

BPmod2 <- lm(sysBP ~ factor(male) + age + factor(education) + factor(currentSmoker)+ BMI +
               glucose, data = framingham) 
summary(BPmod2)



CHDrisk <- glm(TenYearCHD ~  factor(male) + age + factor(currentSmoker)+ BMI +
                 glucose + diaBP + sysBP + BPMeds, data = framingham, family = "binomial")
